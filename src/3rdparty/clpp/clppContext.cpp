...
void clppContext::setup(unsigned int platformId, unsigned int deviceId)
{
	...
	clStatus = clGetDeviceIDs(clPlatform, CL_DEVICE_TYPE_GPU, 0, NULL, &devicesCount);
	assert(clStatus == CL_SUCCESS);
	assert(devicesCount > 0);
	
	cl_device_id* devices = new cl_device_id[devicesCount];
	clStatus = clGetDeviceIDs(clPlatform, CL_DEVICE_TYPE_GPU, devicesCount, devices, NULL);
	assert(clStatus == CL_SUCCESS);

    //Get the device with Maximum Flops
    int max_flops_device = min(deviceId, devicesCount-1);
    for(int i=0; i < devicesCount; i++) {
        char deviceName[500];
        clStatus = clGetDeviceInfo(devices[i], CL_DEVICE_VENDOR, 500, deviceName, NULL);
        if(strstr(deviceName, "NVIDIA") || strstr(deviceName, "AMD")) {
            max_flops_device = i;
            break;
        }
    }

    clDevice = devices[max_flops_device];
    delete [] devices;
    devices = NULL;
